...
double _sequence_delay_L_mutual_entropy(std::vector<uint32_t> words, int L){
    std::map<uint64_t, uint32_t> word_counter;
    std::map<uint32_t, uint32_t> word_joint_counter;
    std::map<uint64_t, double> word_possibility;
    std::map<uint32_t, double> word_joint_possibility;
    ...
    double entropy = 0.0;
    for(auto& map_item: joint_possibility){
        ...
        double p_j = word_possibility.count(symbol_2) > 0 ? word_possibility[symbol2] : 0.0;
